...
export class SummaryComponent implements OnInit, AfterViewInit {
  ...
  onKeyupReal(item, args) {
    if (args.keyCode === 13) {
      if (item.position === 'A') {
        ...
        this.changScanStatusFocus('A', false);
        this.changScanStatusFocus('B', true);
      }
      this.changeReal(item.position, args.target.value);
      this.changeFocusStatus(item.code, false, false);

      // if Chemical is B, focus in chemical C
      if (item.position === 'B') {
        this.changScanStatusFocus('B', false);
        this.changScanStatusFocus('C', true);
      }

      // if Chemical is C, focus in chemical D
      if (item.position === 'C') {
        this.changScanStatusFocus('C', false);
        this.changScanStatusFocus('D', true);
      }
      // if Chemical is D, focus in chemical E
      if (item.position === 'D') {
        this.changScanStatusFocus('D', false);
        this.changScanStatusFocus('E', true);
      }
  ...
  changScanStatusFocus(position, status) {
  ...
  changeReal(position, real) {
    for (const i in this.ingredients) {
      if (this.ingredients[i].position === position) {
        this.ingredients[i].real = this.toFixedIfNecessary(real, 2);;
        break; // Stop this loop, we found it!
      }
    }
  }
